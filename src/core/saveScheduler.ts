...
import type { BlockSpan, JSONCanvas } from './types';
...
export interface SaveSchedulerOptions {
	...
	serialise: (ctx: SaveContext, canvas: JSONCanvas) => string;
...
export function createSaveScheduler(opts: SaveSchedulerOptions): SaveScheduler {
	...
	const runOnce = async (canvas: JSONCanvas): Promise<void> => {
		...
		if (!ctx) return;
		const newBody = opts.serialise(ctx, canvas);
	...
	return {
		schedule(canvas: JSONCanvas): void {
			if (inflight) {
				...
				// rather than queuing is the coalescing. We store the
				// reference, not a clone: if the host mutates the canvas
				// in-place between schedule and persist (e.g., a follow-on
				// drag updating x/y on the same object), the saved body
				// reflects the latest mutation — usually what we want.
				pending = canvas;
				return;
			}
