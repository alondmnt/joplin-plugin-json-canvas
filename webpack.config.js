...
// LOCAL PATCH: entry overridden from `./src/index.ts` to `./src/joplin/index.ts`
// so the plugin host code lives behind the same module boundary as everything
// else under `src/joplin/`. The canvas core in `src/core/` stays Joplin-free.
const pluginConfig = { ...baseConfig, entry: './src/joplin/index.ts',
	resolve: {
		alias: {
...
// LOCAL PATCH (do not lose on `yo joplin --update`):
// Extra scripts run inside webview iframes (browser context), not in the plugin's
// node sandbox. Override the base `target: 'node'` so DOM types resolve and the
// browser bundle shape is correct. Also add `conditionNames` including 'import'
// so ESM-only npm packages (e.g. json-canvas-viewer) resolve under webpack 5's
// exports-condition matching, which would otherwise pick the unavailable
// 'require' branch.
const extraScriptConfig = {
	...
	resolve: {
		...
		fallback: moduleFallback,
		extensions: ['.js', '.tsx', '.ts', '.json'],
		conditionNames: ['import', 'webpack', 'production', 'browser', 'default'],
	},
